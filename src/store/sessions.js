...
const sessions = {
  ...
  getters: {
    allMenteeSessions: state => state.menteeSessions,
    allMentorSessions: state => state.mentorSessions,
  ...
  mutations: {
    SET_MENTEE_SESSIONS(state, mentors) {
      state.menteeSessions = mentors
    },
    SET_MENTOR_SESSIONS(state, mentees) {
      state.mentorSessions = mentees
    },
  },
  actions: {
    async loadMenteeSessions({ commit }, menteeId) {
      let data = []
      await firebase
        .firestore()
        .collection('sessions')
        .where('fromMenteeId', '==', menteeId)
        .get()
        .then(result => {
          result.forEach(doc => {
            data.push(doc.data())
      ...
      console.log(data)
      commit('SET_MENTEE_SESSIONS', data)
      return Promise.resolve()
    },
    async loadMentorSessions({ commit }, mentorId) {
      let data = []
      await firebase
        .firestore()
        .collection('sessions')
        .where('toMentorId', '==', mentorId)
        .get()
        .then(result => {
          result.forEach(doc => {
            data.push(doc.data())
      ...
      console.log(data)
      commit('SET_MENTOR_SESSIONS', data)
      return Promise.resolve()
    },
